...
namespace ego_planner
{
  ...
  void EGOPlannerManager::initPlanModules(ros::NodeHandle &nh, PlanningVisualization::Ptr vis)
  {
    ...
    visualization_ = vis;
    OTP_pub = nh.advertise<geometry_msgs::PoseArray>("/OPT_points", 2);
      ...
      const bool flag_randomPolyTraj, const bool touch_goal)
  {
    ...
    if (flag_success)
    {
      ...
      geometry_msgs::PoseArray OTP;
      OTP.header.frame_id="world";
      geometry_msgs::Pose P ;
      for (int i = 0 ; i < cstr_pts.cols(); i++){
      P.position.x = cstr_pts.col(i).transpose()(0);
      P.position.y = cstr_pts.col(i).transpose()(1);
      P.position.z = cstr_pts.col(i).transpose()(2);
      OTP.poses.push_back(P);
      }
      OTP_pub.publish(OTP);
